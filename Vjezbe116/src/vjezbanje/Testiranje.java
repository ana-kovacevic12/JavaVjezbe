...
public class Testiranje {
	...
	return;
		...
		proizvodi.add(new Telefoni(opis, sifra, uvozna, os, vel));
		...
		} else if (p instanceof Telefoni) {

		tip = "Telefon";
		vel = ((Telefoni) p).getVelicina_Ekrana();
		sifraTip = "TE";


		} else if (p instanceof TV) {
		tip = "TV";
		vel = ((TV) p).getVelicina_ekrana();
		sifraTip = "TV";
		}


		double maloprod = p.obracun_maloprodajne_cijene (sifraTip, vel);


		System.out.printf("Tip: %s, Opis: %s, Sifra: %d, Uvozna: %d, Maloprodajna: %.2f\n",


		tip, p.getSifre_proizvoda(), p.getOpis_proizvoda(), p.getUvozna_cijena_proizvoda(), maloprod);}
		...
		|| (trazeni.equals("Telefon") && p instanceof Telefoni)
		...
		p.getOpis_proizvoda(), p.getSifre_proizvoda(), p.getUvozna_cijena_proizvoda());
